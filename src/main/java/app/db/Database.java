...
public class Database {
  ...
  private void initSchema() throws SQLException {
    try (Statement stmt = connection.createStatement()) {
      ...
      safeAddColumn(stmt, "ALTER TABLE files ADD COLUMN pathScore REAL NOT NULL DEFAULT 0.0");
      safeAddColumn(stmt, "ALTER TABLE files ADD COLUMN dominantColor TEXT");

      stmt.execute("CREATE INDEX IF NOT EXISTS idx_files_extension     ON files(extension)");
      stmt.execute(
          "CREATE INDEX IF NOT EXISTS idx_files_lastModified  ON files(lastModified DESC)");
      ...
      stmt.execute("CREATE INDEX IF NOT EXISTS idx_files_dominantColor ON files(dominantColor)");
    }
  }

  private void safeAddColumn(Statement stmt, String sql) {
    try {
      stmt.execute(sql);
